...
namespace ChatProject.Web.Controllers
{
    public class UserFriendsController : Controller
    {
        ...
        //public List<User> SearchUser(string userName)
        {
            ...
            //userRequests = _requestRepository.GetAll().Where(item => item.ToId == userId && item.Status == RequestStatus.New)
            //    .Select(item => item.FromId).ToList();
        ...
        //public List<User> SearchUser(string userName)
        {
            List<Message> messages = _messageRepository.GetAll().Where(item => (item.ToId == userId && item.FromId == withWhomId) || (item.ToId == withWhomId && item.FromId == userId)).OrderBy(item=> item.Date).ToList();
